...
SAMPLE_RATE = 475           # Hz - hardware controlled by ADS1115
LO_PLUS_PIN = 14  # physical pin 8
LO_MINUS_PIN = 15 # physical pin 10
ADS_CHANNEL = 0
PRINT_INTERVAL = 0.1  # Print every 0.1 seconds
SAVE_INTERVAL = 5000  # Save to CSV every 5000 samples (~10.5 sec at 475 Hz)
...
ads.data_rate = SAMPLE_RATE  # Hardware-controlled sample rate
...
def record_ecg():
    ...
    print(f"⚡ Hardware-timed sampling at {SAMPLE_RATE} Hz")
    ...
    while running:
        ...
        if pause_start is not None:
            ...
            start_time += pause_duration  # Adjust start time to account for pause
            pause_start = None
            print("✓ Electrodes reconnected. Resuming...")

        # Hardware-timed sample (ADC controls the rate)
